...
use smoltcp::iface::{ReassemblyBuffer, InterfaceBuilder, Routes, SocketSet};
use smoltcp::phy::Medium;
use sio_smoltcp::device::SansIO;
...
fn main() {
	...
	let ipv4_out_packet_cache = vec![0u8; 0];
	let mut builder = InterfaceBuilder::new()
		.ip_addrs(ip_addrs)
		.ipv4_reassembly_buffer(ipv4_frag_cache)
		.ipv4_fragmentation_buffer(ipv4_out_packet_cache);
	...
	/*for i in 0..10{
		...
		match packet {
			...
			None => {
				io::stdout().write_all(b"no packet\n");
			}
		}
	}*/
}
